...
namespace kalman
{
    ...
    public:
        ...
        EKF(const Eigen::VectorXd &initial_state, const Eigen::MatrixXd &initial_cov,
            const Eigen::VectorXd &proc_noise_std, const Eigen::VectorXd &obs_noise_std);
        ...
        void setObservationFunction(const std::function<Eigen::VectorXd(const Eigen::VectorXd &, double)> &func);
        const std::function<Eigen::VectorXd(const Eigen::VectorXd &, double)> &getObservationFunction() const;

        // Getter and setter for the observation Jacobian.
        void setObservationJacobian(const std::function<Eigen::MatrixXd(const Eigen::VectorXd &, double)> &jac);
        const std::function<Eigen::MatrixXd(const Eigen::VectorXd &, double)> &getObservationJacobian() const;
